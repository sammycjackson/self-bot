...
class Utility:
    ...
    async def cpres(self, ctx, status : str, Type:str = None, *, message:str = None):
        ...
        if message is None:
            ...
            if Type == "playing":
                ...
                await self.bot.change_presence(status=stats[status], activity=discord.Streaming(name=message, url=https://twitch.com/%7Bmessage%7D), afk = True)
    ...
    async def google(self, ctx, numberOfResults:int, *, query):
        ...
        try:
            ...
            next_two = [x[0] for x in entries[:numberOfResults]]
